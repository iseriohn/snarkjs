...
async function importResponseNoOrigin(oldPtauFilename, contributionFilename, newPTauFilename, name, importPoints, logger) {
    ...
    // Disabling this check because the ppot generated by kobi is imported as 
    // a whole so contributionPreviousHash doesn't match with lastChallengeHash 
    // our artificial initial oldPtauFile. The initial oldPtaFile is deterministic 
    // so can be reproduced and trusted.
